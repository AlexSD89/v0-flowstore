...
from pocketcorn.services.collectors.mindspider import MindSpiderWeiboService


class WeiboAdapter(BaseAdapter):
    platform_id = "weibo"
    display_name = "Weibo"
    supported_categories = ("product", "marketing", "community", "funding", "hiring")

    def __init__(self) -> None:
        super().__init__()
        self._service = MindSpiderWeiboService()

    def collect(self, mission_criteria: Dict[str, object]) -> List[RawSignal]:
        self._ensure_not_rate_limited()
        signals = self._service.collect(mission_criteria)
