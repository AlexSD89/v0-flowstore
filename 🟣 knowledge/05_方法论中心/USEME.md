...
4. 若涉及方法论治理/归档任务，请同步查看 `todo-20251115-methodology-cleanup.md` 的最新勾选状态。
...
## 7. 标准协作提示词（2025-10-14 模板整合）

> 以下模板来自 2025-10-14《AI协作提示词模板》，已根据当前七大方法论域精简。

### 基础协作 / 代码执行
```markdown
你是 LaunchX 的 AI 协作者，当前目录：{{path}}。

步骤：
1. 读取根级 `CLAUDE.md`、本域 `README/RULES/USEME`
2. 查找 `support_modules` 是否已有可复用能力
3. 说明 Collect→Model→Compare 的关键结论与风险
4. 输出前记录互链与验证
...
```markdown
请为 {{任务}} 设计技术方案，遵循：
- 复用 `🛠️ 技术开发方法论/` 与 `support_modules/dev`
- 禁止 barrel import，引用具体路径
- 若需 Hook/MCP，请引用对应 USEME/RULES
- 输出：架构概览 → 关键组件 → 风险/验证
```
...
```markdown
请对 {{研究主题}} 进行深度研究：
- 启动 BMAD 5 通道（主题/相关/动态/专家/趋势）
- 记录每个通道的来源与可信度（A/B/C）
- 生成结构化报告：洞察、证据、建议、TODO
- 缺少数据时使用 `TODO｜待补充 + 来源`
```
