...
### AI Context 实施指南（2025-10-14 摘要）

> 来源：2025-10-14《AI Context 实施方法论（掘金最佳实践）》，已与本工程化体系融合，仅保留可执行要点。

#### 三层文档优先级
- **Layer 1｜全局路标**：`CLAUDE.md` 负责协作分级、Phase 0 Checklist、自动化触发条件。
- **Layer 2｜模块规范**：各域 `USEME.md`、`RULES.md` 定义能力清单与红线；所有任务进入 `/plan → /do` 前必须加载对应 Use Guide。
- **Layer 3｜资产仓**：`memory-bank/support_modules/*` + 业务目录文档，引用时记录 `path:line` 和验证方式，缺口写 `TODO｜待补充 + 来源`。

#### 跨仓 Context 连接方式
- **.cursorrules**：写明“查 CLAUDE → 查 USEME → 复用 support_modules”的默认流程，并标注该目录是否为 monorepo 子仓。
- **文档地图**：在根级 CLAUDE/README 内列出各包关键路径（Hook、Skill、模板），规范导入方式（禁止 barrel import）。
- **参数表与示例**：所有 USEME 示例使用真实路径、完整错误处理，避免“简化示例”导致 AI 复制错误模式。

#### 最佳实践 vs. 反模式
| 推荐动作 | 意义 | 反模式 | 后果 |
| --- | --- | --- | --- |
| 只写“如何使用” | 让 AI 站在使用者视角 | 解释实现/原理 | AI 尝试重写能力 |
| 给出完整示例 | 让质量基线可复制 | 省略校验、日志 | 复制错误示例 |
| 约束 ≤5 条 + 示例 | 更易被记忆与执行 | 大段规则清单 | 造成执行畏缩、效率低 |

#### 1 小时落地清单（可放入 `/spec`）
1. **文档体系（20min）**：补齐 CLAUDE 入口、公共包 USEME、跨仓地图。
2. **规范统一（15min）**：验证导入方式、Hook 命名、禁止事项，必要时更新 `.cursorrules`。
3. **工具配置（10min）**：部署质量脚本、日志采集、监控 Hook。
4. **验证试跑（15min）**：同一需求做“无上下文 vs. 规范执行”对照实验并记录差异。

#### 评估指标
- **复用率 ≥80%**：代码审查统计引用 support_modules 的比例。
- **文档命中率 ≥90%**：日志记录 Phase 0 是否加载 CLAUDE/USEME。
- **错误率 ↓70%**：对比引入规范前后的重复开发或基础错误数量。
