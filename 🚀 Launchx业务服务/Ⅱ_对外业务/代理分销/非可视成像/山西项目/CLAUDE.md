...
| Assess | 判定 Level S/M/L，记录理由与升级条件 | 读取判级结果 | `CLAUDE.md:91-110` |
| Gather | 执行 rg/fd 检索、调用 Skills/MCP，产出引用清单 | 根据需要执行本地命令 | `AGENTS.md:31-78`、`RULES.md:286-292` |
| Deliver | 写明 Dev Docs 更新点、验证计划、Hook/Skill 日志 | 执行 Codex 指令，更新仓内文件 | `CLAUDE.md:125-160` |
...
| --- | --- | --- | --- |
| 快速问答（Level S） | `rg`/`fd` 检索 + `rube` 校验，Summary 写 mini plan，并把产物写入 auto-generated 目录 | `rg "<关键词>" -g '<file>'`、`rube ask ...` | `AGENTS.md`“场景触发”表、`AGENTS.md:104-111` |
...
```markdown
## Assess
- 等级：Level [S/M/L]（依据 `CLAUDE.md:91-110`），升级条件：[描述]
- 上下文：已读取文档/资产
- 关键风险：……

## Gather
- 本地引用：@path:line …
- Skills/MCP：工具 + 目的 + 输出/日志目录
- 待验证/缺口：TODO｜待补充 + 来源

## Deliver
- 对 Codex 指令：命令 / 预期 / 风险
- Dev Docs 更新点：plan/context/tasks
- Hook/Skill 记录：参考下节示例
```
...
### Hook/Skill 调用记录模板
在 Summary 或 plan/context 中引用以下结构，确保 Codex 可以复盘命令与产出：
```markdown
| 类型 | 名称 | 命令/触发脚本 | 输出目录/日志 | 目的与风险 | 下一步 |
| --- | --- | --- | --- | --- | --- |
| Hook | dev-docs-workflow | `node .claude/hooks/dev-docs-workflow/hook.js --project=...` | .claude/output/dev-docs/... | 初始化三文件，风险：模板失配 | 待 Codex 校验 |
| Skill | project-architect | `skill run project-architect --context=...` | logs/skills/project-architect-<ts>.log | 生成方案对比，风险：引用缺失 | 待 Codex 粘贴结果 |
```
若命令失败，仍需保留记录并补充替代方案或 TODO。
