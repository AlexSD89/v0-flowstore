...
> 本 RULES.md 用作 AGENTS/CLAUDE 的操作补充：当需要命令模板、Phase 0 细则、Hook/Skill 调度或输出前校验时，优先引用本文件对应段落（path:line），并在 Summary 保留命令与日志路径。
...
#### 强制认知加载流程
分为【Must】与【Nice-to-have】，其中 Must 必须全部满足才可进入计划阶段：

```bash
Must
[ ] 确认 Claude 身份（不直接执行命令）与交接边界
[ ] 工程基础设施检查：PM2监控 / 增量构建 / 技能渐进披露 / Hook 配置
[ ] 加载根级指挥文档：CLAUDE.md · AGENTS.md · RULES.md
[ ] 加载任务域文档：README / CLAUDE / RULES / USEME
[ ] 检索 memory-bank 与 support_modules，列出可复用资产
[ ] Memory-Bank 路径对齐检查（`🛠️ 系统管理/memory-bank/README.md:31-63`）
[ ] 任务复杂度评估：关键词分类 + Level S/M/L + 是否需 Skills/MCP
[ ] 工程能力验证：自动化工具 / 可观测性 / 质量门控
[ ] 文档质量检查：frontmatter、last_update、路径有效性
[ ] 风险标记：权限 / 数据 / 时间节点 / 依赖
[ ] 最终确认：未完成任何 Must 项不得进入计划或执行阶段

Nice-to-have
[ ] `rg "最佳实践"` / `rg "USEME"` 记录检索结果
[ ] 验证脚本、模板可访问性
[ ] 记录路径对齐检查日志与修复计划
[ ] 若缺少 Dev Docs，准备 “待 Claude 初始化 dev-docs/<project>/” TODO
```
...
```bash
# 重要规程快速定位
Phase 0认知加载:     @RULES.md:578-618
MCP工具使用规范:     @RULES.md:360-394
文档生成质量标准:   @RULES.md:397-454
大型文件开发规程:   @RULES.md:501-575
错误处理纠正:       @RULES.md:747-859
二级规则生成标准:   @RULES.md:1346-1408
更新二级规则流程:   @RULES.md:1409-1497
Skills调用规程:     @RULES.md:851-900
禁止操作检查:       @RULES.md:1500-1550
Claude能力边界:     @CLAUDE.md:27-207
```
