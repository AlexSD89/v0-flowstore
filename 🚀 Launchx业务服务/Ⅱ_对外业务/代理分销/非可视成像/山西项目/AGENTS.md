...
| Level 1：执行框架 | 5 步认知 + Dev Docs 映射 | `CLAUDE.md:125-176`、`RULES.md:55-140` | `/spec` or plan/context/tasks 的结构 |
...
> `RULES.md` 是 Level 0-1 的操作补充层：凡是需要命令模板、Hook 说明或 Phase 0 细节，统一引用 `RULES.md` 对应段落并在 Summary 中保留命令 + 日志路径。
...
- 必做 `memory-bank` 路径对齐检查（`🛠️ 系统管理/memory-bank/README.md:31-63`），异常时立即停止执行并记录修复计划。
...
| 模块 | 要点 | 依据 |
| --- | --- | --- |
| plan.md / mini plan | Collect/Model/Compare 输出写入 plan，小任务用 mini plan 支撑；无三文件时保持在 Summary 中同步 | `CLAUDE.md:173-185` |
| context.md / SESSION PROGRESS | 用 ✅/🟡/⚠️ 标注状态、关键文件与 Quick Resume，便于交接 | `CLAUDE.md:173-185` |
| tasks.md | 分阶段列任务、责任人、验收标准，并同步阻塞项 | `CLAUDE.md:173-185` |
| Summary 模板 | 统一使用 `Summary / Testing / Next Steps`，Next Steps 写明“待 Claude：<命令/风险>”与 Codex TODO；记录产物生成目录（auto-generated 或项目目录） | `RULES.md:521-540` |
| 验证与互链 | 记录命令/日志/引用；发现需要沉淀时提出 memory-bank 更新建议，等待 Claude 落地 | `🛠️ 系统管理/memory-bank/README.md:31-63` |
...
## 8. 输出与引用规范
- **Summary 模板**：沿用第 6 节的 `Summary / Testing / Next Steps` 结构，未验证需说明风险与代办。  
...
- **草稿治理**：Level S 或一次性产出放在 `/Users/dangsiyuan/Documents/obsidion/launch x/🤖 AI生成 auto-generated/YYYYMMDD/<slug>/`；Level M/L 项目或多轮交付直接在对应项目目录维护。24h 内迁移或删除并在 Summary 标注处理结果。
